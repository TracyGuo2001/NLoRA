...
def test_hendrycks_math(model, data_path, output_json, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    hendrycks_math_ins = []
    ...
    llm = LLM(model=model,tensor_parallel_size=tensor_parallel_size, trust_remote_code=True)
    res_completions = []
    ...
    output_json = output_json+model.split('/')[-1] + model.split('/')[-2]+'_output.json'
...
def parse_args():
    ...
    parser.add_argument("--output_json", type=str, default="/home/guochenlu/guo/PN/evaluate/MATH/MATHoutput/")  # JSON output path
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    test_hendrycks_math(model=args.model, data_path=args.data_file, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size,output_json=args.output_json)

    # test_hendrycks_math(
    #     model= "",
    #     data_path= "data/MATH_test.jsonl", 
    #     output_json = "MATHoutput/",
    #     start=0, 
    #     end=MAX_INT, 
    #     batch_size=60, 
