    ...
    class Linear8bitLt(torch.nn.Module, LoraLayer):
        ...
        def get_delta_weight(self, adapter):
            if self.lora_N:
                return (
                transpose(
                    self.lora_B[adapter].weight @ self.lora_N[adapter].weight @  self.lora_A[adapter].weight,
                    False,
                )
        ...
        def _mixed_batch_forward(
            ...
            for i, active_adapter in enumerate(unique_adapters):
                ...
                if self.lora_N[active_adapter]:
                    lora_N = self.lora_N[active_adapter]
                    output = lora_B(lora_N(lora_A(dropout(sub_batch)))) * scaling
                else:
            ...
            if self.disable_adapters:
                ...
                for active_adapter in self.active_adapters:
                    ...
                    if not self.use_dora[active_adapter]:
                        if self.lora_N[active_adapter]:
                            lora_N = self.lora_N[active_adapter]
                            output = lora_B(lora_N(lora_A(dropout(x)))) * scaling
                        else:
