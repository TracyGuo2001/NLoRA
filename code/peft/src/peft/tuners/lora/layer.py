...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        # the middle matrix in Nlora 
        self.lora_N = nn.ModuleDict({})
        self.lora_embedding_N = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        elif init_lora_weights == "n_lora":
            with gather_params_ctx(self.get_base_layer().weight):
                self.nlora_init(adapter_name)
        elif init_lora_weights == "s_lora":
            with gather_params_ctx(self.get_base_layer().weight):
                self.slora_init(adapter_name)
        ...
        self.set_adapter(self.active_adapters) # 如果是adapter_names中需要训练的层，则开启梯度传播，否则关闭
    ...
    def nlora_init(self, adapter_name):
        r = self.r[adapter_name]
        if self.lora_A and not self.lora_N: 
            # first nlora layer being added, add lora_N to the list of learnable parameters
            self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_N",)
            Nlayer_type = type(self.get_base_layer())
            self.lora_N[adapter_name] = Nlayer_type(self.r[adapter_name], self.r[adapter_name], bias=False)

        if self.lora_embedding_A and not self.lora_embedding_N:
            self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_embedding_N",)
            weight_N = torch.randn((r, r))
            self.lora_embedding_N[adapter_name] = nn.Parameter(weight_N)
            
        if adapter_name in self.lora_A.keys():    
            # Nystrom
            param = self.get_base_layer().weight

            lora_N_matrix = param[:r, :r]
            lora_A_matrix = param[:r, :]         
            lora_B_matrix = param[:, :r]       
            with torch.no_grad():
                self.lora_A[adapter_name].weight.copy_(lora_A_matrix)
                self.lora_B[adapter_name].weight.copy_(lora_B_matrix)
                self.lora_N[adapter_name].weight.copy_(lora_N_matrix)

        if adapter_name in self.lora_embedding_A.keys(): 
            # Nystrom
            param = self.get_base_layer().weight

            # lora_embedding_N_matrix = torch.linalg.pinv(param[:r, :r])
            lora_embedding_N_matrix = param[:r, :r]
            lora_embedding_A_matrix = param[:r, :]        # 右上部分
            lora_embedding_B_matrix = param[:, :r]        # 左下部分
            with torch.no_grad():
                self.lora_embedding_A[adapter_name].weight.copy_(lora_embedding_A_matrix)
                self.lora_embedding_B[adapter_name].weight.copy_(lora_embedding_B_matrix)
                self.lora_embedding_N[adapter_name].weight.copy_(lora_embedding_N_matrix)

    def slora_init(self, adapter_name):
        r = self.r[adapter_name]
        if self.lora_A and not self.lora_N: 
            # first nlora layer being added, add lora_N to the list of learnable parameters
            self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_N",)
        ...
        if adapter_name in self.lora_A.keys():    
            nn.init.kaiming_uniform_(self.lora_A[adapter_name].weight, a=math.sqrt(5))
            nn.init.zeros_(self.lora_B[adapter_name].weight)
            # nn.init.zeros_(self.lora_N[adapter_name].weight) #grad=0
            # nn.init.normal_(self.lora_N[adapter_name].weight, std=1 / self.r[adapter_name])
            nn.init.kaiming_uniform_(self.lora_N[adapter_name].weight, a=math.sqrt(5))

        if adapter_name in self.lora_embedding_A.keys(): 
            nn.init.zeros_(self.lora_embedding_A[adapter_name])
            nn.init.normal_(self.lora_embedding_B[adapter_name])
            nn.init.normal_(self.lora_embedding_N[adapter_name])
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.lora_N:
                lora_N = self.lora_N[active_adapter]
                lora_output = lora_B(lora_N(lora_A(sub_batch))) * scaling 
            else:
...
class Linear(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.lora_N:
            weight_N = self.lora_N[adapter].weight
        ...
        if self.lora_N:
            output_tensor = transpose(weight_B @ weight_N @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]
        else:
        ...
        if cast_to_fp32:
            ...
            if self.lora_N:
                self.lora_N[adapter].weight.data = weight_N.to(dtype)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    if self.lora_N:
                        # xTrans = nn.Linear(x.shape[-1], x.shape[-1] - self.r[active_adapter]).to(x.device)
                        # x = xTrans(x)
                        result = result + lora_B(lora_N(lora_A(dropout(x)))) * scaling
                    else:
...
class Embedding(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.lora_embedding_N:
            weight_N = self.lora_embedding_N[adapter]
        ...
        if self.lora_embedding_N:
            output_tensor = transpose(weight_B @ weight_N @ weight_A, True) * self.scaling[adapter]
        else:
        ...
        if cast_to_fp32:
            ...
            if self.lora_embedding_N:
                self.lora_embedding_N[adapter] = weight_N.to(dtype)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.lora_embedding_N:
                embedding_N = self.lora_embedding_N[active_adapter] 
                result[sub_batch_indices_list[i]] += (after_A @ embedding_N @ embedding_B) * scaling
            else:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if self.lora_embedding_N:
                        embedding_N = self.lora_embedding_N[active_adapter]
                        result = result + (after_A @ embedding_N @ embedding_B) * scaling
                    else:
...
class _ConvNd(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.get_base_layer().weight.size()[2:4] == (1, 1):
            # conv2d 1x1
            output_tensor = (weight_B.squeeze(3).squeeze(2) @ weight_N @ weight_A.squeeze(3).squeeze(2)).unsqueeze(2).unsqueeze(
                3
            ) * self.scaling[adapter]
        else: 
            if self.lora_N: 
                weight_B = weight_N @ weight_B
            output_tensor = (
                self.conv_fn(
