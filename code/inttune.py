...
class TrainingArguments(transformers.TrainingArguments):
    ...
    adapter_name_or_path: Optional[str] = field(default=None, metadata={"help": (
        "Pre-initialized adapter path; when this is not None, the following arguments are ignored."), }, )
    init_lora_weights: Literal[True, "pissa",'n_lora','s_lora',"gaussian"] = field(default=True,
                                                      metadata={"help": ("'n_lora' -> NLoRA; 's_lora' -> SLoRA;`True -> LoRA; pissa` -> PiSSA"), }, )
    target_modules: Optional[str] = field(default="q_proj,v_proj,k_proj,o_proj,gate_proj,down_proj,up_proj")
    lora_rank: Optional[int] = field(default=8)
    lora_alpha: Optional[float] = field(default=8)
    ...
    merge: Optional[bool] = field(default=False, metadata={
        "help": "Merge the adapter to the base model"}, )
...
def build_model(script_args, checkpoint_dir):
    ...
    print(f"{script_args.init_lora_weights} Execution time: {elapsed_time:.4f} seconds")
    logger.info(f"{script_args.init_lora_weights} Execution time: {elapsed_time:.4f} seconds")
...
def get_WanDB_project_name(path):
    path_to_project_name = {
        "meta-math/MetaMathQA": "NLORA",
        "WizardLMTeam/WizardLM_evol_instruct_70k": "NLORA_Wizardoutput",
        "m-a-p/CodeFeedback-Filtered-Instruction":'NLORA_CodeFeedback',
    }
    return path_to_project_name.get(path, "Unknown_Project")
...
def train():
    ...
    project_name = get_WanDB_project_name(script_args.data_path)

    if script_args.local_rank <= 0: 
        run_name = f"{time.strftime('%m%d_%H%M')}"
        wandb.init(
            # project="NLORA", 
            project= project_name, 
            group =  f"{script_args.model_name_or_path.split('/')[-1]}_{script_args.init_lora_weights}_on_{time.strftime('%m%d')}_lr{script_args.learning_rate}_r{script_args.lora_rank}",
            name=run_name,    
            mode="online"
        )
